...
- **오직 knockout(r32/r16/qf/sf/final)만** 손댄다. 그중에서도:
  - 이미 `status:"FT"` 이고 **스코어와 득점자(`goals`)가 모두 채워진** 경기는 **다시 건드리지 않는다**(확정 완료 — 재검색 불필요).
  - **⭐ 예외(득점자 1회 백필)**: `status:"FT"` 인데 `goals` 가 **빈 배열(`[]`)** 인 경기는, 그 경기의 **득점자·득점 시간을 웹에서 찾아 딱 한 번 채운다**. 한 번 채워지면 다음부터는 다시 검색하지 않는다.
    - 단 **0-0 무득점 경기**(homeScore·awayScore 가 모두 0)는 `goals` 가 비는 게 정상이므로 건드리지 않는다.
    - 득점자를 어떤 소스에서도 확인 못 하면 그 골은 지어내지 말고 `player:"미상", minute:null` 로 두거나 비워둔다(스코어·승자는 유지).
  - `status` 가 `LIVE`/`NS`/`TBD` 인데 **경기 날짜(date)가 오늘 이하**인 경기는 웹에서 결과를 확인해 갱신한다.
...
- **⭐ ESPN 등 영문 소스에서 득점자·시간이 안 나오면 반드시 `naver.com`(네이버 스포츠 sports.news.naver.com, 또는 search.naver.com "○○ vs △△ 경기결과")에서 확인해 채운다.** 영문에서 못 찾았다고 빈 채로 넘기지 말고 네이버를 꼭 한 번 더 뒤진다. 선수명은 기존 data.js 표기와 맞추되(한국 선수만 한글), 네이버에만 있으면 원어/영문 표기로 넣어도 된다.
